...
mod tests {
    ...
    fn foundation_negative_diagnostic_is_stable_and_source_aware() {
        ...
        let child = fs::canonicalize(root.join("examples/foundation_app/invalid/broken.azk"))
            .expect("canonicalize invalid child fixture");
        ...
        assert!(first.contains(&format!(
            " --> {}",
            frontend::diagnostics::display_path(&child)
        )));
    ...
    fn project_semantic_diagnostics_render_the_imported_source() {
        ...
        let canonical_broken =
            fs::canonicalize(&broken_path).expect("canonicalize broken imported module");
        let canonical_main = fs::canonicalize(&main_path).expect("canonicalize root module");
        assert!(first.contains(&format!(
            " --> {}",
            frontend::diagnostics::display_path(&canonical_broken)
        )));
        assert!(first.contains("pub fn answer() -> u64 { let value: u64 = true;"));
        assert!(!first.contains(&format!(
            " --> {}",
            frontend::diagnostics::display_path(&canonical_main)
        )));
        let _ = fs::remove_dir_all(project);
    }
    ...
    fn project_ownership_diagnostics_render_the_imported_source() {
        ...
        let canonical_broken =
            fs::canonicalize(&broken_path).expect("canonicalize broken owning module");
